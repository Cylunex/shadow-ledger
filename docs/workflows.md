...
批量确认最多接受 1000 个明确的 `(record_id, revision)`，先完成全批状态和版本检查，再统一写入
确认时间、审计和 Outbox；任一条冲突时整个事务回滚。网页草稿箱同时保留单条详情确认和删除入口。
...
- 明确支付方式保存为受控标签（ADR 0008），未知留空，不做账户建模；原始商家、渠道和商品文本同时原样进入草稿，
