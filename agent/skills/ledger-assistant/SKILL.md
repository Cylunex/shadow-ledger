...
description: 读取用户已授权的账目与预算摘要，并创建等待 Ledger 用户审核的记账草案。
---

# Ledger Assistant

仅在用户需要查看自己的收支、预算进度或明确要求保存记账草案时使用本 Skill。
...
4. 只有用户明确要求保存时才调用 `ledger.records.draft`，并原样使用用户给出的金额事实。
5. 返回草案引用和 `draft` 状态，说明它尚未成为正式账目，最终确认由 Ledger 用户会话完成。
...
- Skill 只能使用这里列出的 read + draft 能力，不能调用普通浏览器 API 完成确认、撤销或导出。
