...
test('money and selected totals use exact decimal arithmetic',()=>{
  ...
  assert.equal(core.sumAmounts(['9999999999999.9999','0.0001']),'10000000000000.0000');
...
test('network retry keeps key; concurrent same intent shares one request',async()=>{
  ...
  fail=false;
  const results=await Promise.all([core.api('/records',options),core.api('/records',options)]);
...
test('versioned commands do not share an in-flight request across revisions',async()=>{
  ...
  await Promise.all([1,2].map(revision=>core.api('/records/test',{method:'PATCH',
    headers:{'If-Match':String(revision)},body:'{"note":"x"}'})));
...
test('offline queue is opt-in, user-bound and only syncs drafts',async()=>{
  ...
  for(const selector of ['#offline-clear','#offline-sync']){
    nodes.set(selector,{disabled:false,addEventListener:(name,handler)=>events[selector]=handler});
  }
  nodes.set('#offline-count',{textContent:''});
  nodes.set('#offline-enable',{checked:false});
  const sent=[];
  globalThis.fetch=async(path,options)=>{
    if(path.endsWith('/me'))return {ok:true,status:200,json:async()=>({owner_id:'alice'})};
    sent.push(options);
